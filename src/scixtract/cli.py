...
def generate_markdown(result: ExtractionResult, pdf_path: Path) -> str:
    ...
    # Process sections
    section_order = ["abstract", "introduction", "methods", "results", "discussion", "conclusion", "main"]
    ...
    for section_type in section_order:
        if section_type in result.sections:
            ...
            for item in section_data:
                ...
                if item.get("structured") and "extraction_error" not in item["structured"]:
                    lines.extend([
...
def extract_command(args: argparse.Namespace) -> None:
    ...
    output_dir_path = (makefile_args.get('output_dir') or 
    ...
    bib_file = None
    bib_file_path = (makefile_args.get('bib_file') or 
                    getattr(args, 'bib_file', None))
    if bib_file_path:
    ...
    model = (makefile_args.get('model') or 
    ...
    try:
        ...
        update_knowledge = (makefile_args.get('update_knowledge') or 
                           getattr(args, 'update_knowledge', None))
