...
class TestKnowledgeTracker:
    ...
    def test_init_database(self):
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='concept_network'")
        assert cursor.fetchone() is not None
    ...
    def test_add_extraction_result(self):
        ...
        cursor.execute("SELECT COUNT(*) FROM keywords WHERE cite_key = ?", ("test2024",))
